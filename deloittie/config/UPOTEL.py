...
class API(BaseConfig):
    ...
    def register(self,method=None,need=None,thisApi=None,key=None,add1=None): #注册用户
        thisApi = eval(thisApi)
        header = self.headers(method)
        form1 = """
              <soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
              <soap:Body>
              <UserRegisterVJ xmlns="http://tempuri.org/">
                  """

        form2 = """
                  <JsonData>{"MasterSecret": "CTGi*7d54Fs*eruieud545Jgsdudb===Yhdt6", "AppEstateId": "5F3508AE245B6F57", "Version": "1.0.0",
                  "DeviceInfo": "Redmi Note 4|7.0|WIFI|1.0.0|AArch64 Processor rev 4 (aarch64)", "ClientType": "2", "Language": 2, "NickName":%s,
                  "EmailAccount":%s,"UsersPassword":%s,"CountryCode":%s,"PhoneNo":%s,"PromoCode":%s}</JsonData>"""%(json.dumps(thisApi["NickName"]),json.dumps(thisApi["EmailAccount"]),json.dumps(thisApi["UsersPassword"]),json.dumps(thisApi["CountryCode"]),json.dumps(thisApi["PhoneNo"]),json.dumps(thisApi["PromoCode"]))
        form3 = """
                           </UserRegisterVJ>
                  </soap:Body>
                  </soap:Envelope>
