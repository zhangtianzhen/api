...
class BaseQuery(object):
    ...
    def insert(self,testid,expect,result,apireturn,times):
        self.cursor.execute("insert into request(testid,expect,result,apireturn,times) values(%s,%s,%s,%s,%s)",[testid,expect,result,apireturn,times])
        self.conn.commit()

    def update(self,testid,expect,result,apireturn,times):
        self.cursor.execute("update request set expect=%s,result=%s,apireturn=%s,times=%s where testid=%s",[expect,result,apireturn,times,testid])
        self.conn.commit()
    def allCase(self):
        self.cursor.execute("select count(testid) from request")
